...
namespace rc 
{
	...
	bool raycast_engine::outside_map_bounds(point pt)
	{
		return true;
	}
	...
	void raycast_engine::add_wall_texture(texture wall_texture)
	{
	}
	...
	point raycast_engine::find_closest_horizontal_intersection_on_ray(ray ray)
	{
		return point(0, 0);
	}

	point raycast_engine::find_closest_vertical_intersection_on_ray(ray ray)
	{
		return point(0, 0);
	}

	point raycast_engine::find_closest_intersection_on_ray(ray ray)
	{
		return point(0, 0);
	}

	slice raycast_engine::cast_ray(point player_pos, double player_angle, int screen_column)
	{
		return slice(0, 0);
	}
