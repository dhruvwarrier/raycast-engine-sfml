...
#define PROJECTION_FACTOR 23.5*sf::VideoMode::getFullscreenModes()[0].width
...
grid_point inline convert_to_grid_point(int unit_x, int unit_y);
...
slice_info cast_ray(int playerX, int playerY, double player_angle, int screen_column) {
	...
	if (closest_distance == 0) {
		...
		return slice_info(INT_MAX, INT_MAX);
	} else {
		...
		return slice_info(slice_size, (SCREEN_SIZE_Y - slice_size) / 2);
	}
}
...
point emit_and_trace_ray(int first_inter_x, int first_inter_y, int inter_offset_x, int inter_offset_y) {
	...
	while (true) {
		...
		if (outside_map_bounds(current_inter_x, current_inter_y)) {
		...
		else if (MAP_DATA[current_inter_grid_point.x][current_inter_grid_point.y] == 1) {
	...
	if (reached_map_bounds) {
		...
		return point(current_inter_x, current_inter_y);
	}
}
...
double find_closest_distance_to_wall(int playerX, int playerY, point& horiz_intersection, point& vert_intersection) {
	...
	double distance_horiz = sqrt(pow(playerX - horiz_intersection.x, 2) + pow(playerY - horiz_intersection.y, 2));
	double distance_vert = sqrt(pow(playerX - vert_intersection.x, 2) + pow(playerY - vert_intersection.y, 2));
...
grid_point inline convert_to_grid_point(int unit_x, int unit_y) {
	return grid_point(unit_x >> 6, unit_y >> 6);
}
